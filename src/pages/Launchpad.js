...
function Launchpad() {
  ...
  const [globalMintCount, setGlobalMintCount] = useState(1235); // 全局铸造次数状态
  const [lastUpdateTime, setLastUpdateTime] = useState(Date.now()); // 上次更新时间
  ...
  // 模拟全局API - 使用一个全局的数据源
  // const GLOBAL_MINT_API_URL = 'https://api.jsonbin.io/v3/b/your-bin-id'; // 真实API地址（暂时注释）
  
  // 获取全局铸造数据（模拟API调用）
  const fetchGlobalMintData = async () => {
    try {
      // 方案1：使用简单的全局存储（所有用户共享）
      const globalData = localStorage.getItem('satsspark_global_mint_data');
      if (globalData) {
        const parsed = JSON.parse(globalData);
        return {
          count: parsed.count || 1235,
          lastUpdate: parsed.lastUpdate || Date.now()
        };
      }
      
      // 如果没有数据，初始化
      const initialData = { count: 1235, lastUpdate: Date.now() };
      localStorage.setItem('satsspark_global_mint_data', JSON.stringify(initialData));
      return initialData;
      
      // 方案2：真实API调用（注释掉，需要时启用）
      /*
      const response = await fetch(GLOBAL_MINT_API_URL);
      const data = await response.json();
      return {
        count: data.record.mintCount || 1235,
        lastUpdate: data.record.lastUpdate || Date.now()
      };
      */
    } catch (error) {
      console.error('获取全局mint数据失败:', error);
      return { count: 1235, lastUpdate: Date.now() };
    }
  };

  // 更新全局铸造数据
  const updateGlobalMintData = async (newCount) => {
    try {
      const updateData = {
        count: newCount,
        lastUpdate: Date.now()
      ...
      // 方案1：更新本地全局存储
      localStorage.setItem('satsspark_global_mint_data', JSON.stringify(updateData));
      
      // 方案2：真实API调用（注释掉，需要时启用）
      /*
      await fetch(GLOBAL_MINT_API_URL, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'X-Master-Key': 'your-api-key'
        },
        body: JSON.stringify({ mintCount: newCount, lastUpdate: Date.now() })
      });
      */
      
      setGlobalMintCount(newCount);
      setLastUpdateTime(Date.now());
      
      // 触发其他标签页/窗口的更新
      window.dispatchEvent(new CustomEvent('globalMintUpdate', { 
        detail: { count: newCount, timestamp: Date.now() }
      }));
      
    } catch (error) {
      console.error('更新全局mint数据失败:', error);
    }
  };
  ...
  useEffect(() => {
    const loadGlobalData = async () => {
      const data = await fetchGlobalMintData();
      setGlobalMintCount(data.count);
  ...
  useEffect(() => {
    const handleGlobalUpdate = (event) => {
      setGlobalMintCount(event.detail.count);
      setLastUpdateTime(event.detail.timestamp);
    };

    const handleStorageChange = (event) => {
      if (event.key === 'satsspark_global_mint_data' && event.newValue) {
    ...
    window.addEventListener('globalMintUpdate', handleGlobalUpdate);
    window.addEventListener('storage', handleStorageChange);

    return () => {
      window.removeEventListener('globalMintUpdate', handleGlobalUpdate);
      window.removeEventListener('storage', handleStorageChange);
    };
  }, []);

  // 定时轮询更新（可选，用于同步其他用户的铸造）
  useEffect(() => {
  ...
  // 已替换为上面的 updateGlobalMintData 函数
  ...
  const handleTokenLaunch = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        // 更新全局铸造次数（所有用户都能看到）
        await updateGlobalMintData(globalMintCount + mintCount);
  ...
  const projects = [
    {
      ...
      status: 'live',
      raised: globalMintCount * TOKEN_CONFIG.feeAmount,
      target: TOKEN_CONFIG.totalMints * TOKEN_CONFIG.feeAmount,
      participants: globalMintCount,
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  <p className="text-orange-300 font-medium">
                    {globalMintCount.toLocaleString()}/{TOKEN_CONFIG.totalMints.toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
