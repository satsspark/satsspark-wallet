...
function Launchpad() {
  ...
  const [globalMintCount, setGlobalMintCount] = useState(1235); // 添加全局铸造次数状态
  ...
  // 加载全局铸造次数
  useEffect(() => {
    const savedGlobalMintCount = localStorage.getItem('global_mint_count');
    if (savedGlobalMintCount) {
      setGlobalMintCount(parseInt(savedGlobalMintCount));
    }
  }, []);
  ...
  // 更新全局铸造次数
  const updateGlobalMintCount = (newCount) => {
    setGlobalMintCount(newCount);
    localStorage.setItem('global_mint_count', newCount.toString());
  };
  ...
  const handleTokenLaunch = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        // 更新全局铸造次数
        updateGlobalMintCount(globalMintCount + mintCount);
