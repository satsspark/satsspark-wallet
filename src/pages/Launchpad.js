...
function Launchpad() {
  ...
  const TOKEN_CONFIG = {
    ...
    tokensPerMint: 100000, // 每次mint获得的代币数量
    feeAmount: 500, // sats
  ...
  const handleTokenLaunch = async () => {
    // 检查mint是否已结束
    const currentProject = projects.find(p => p.isTokenLauncher);
    if (currentProject && currentProject.status === 'ended') {
      toast.error(i18n.language === 'en' ? 'Minting has ended' : '铸造已结束');
      return;
    }
    ...
    try {
      ...
      if (result) {
        ...
        const totalTokens = TOKEN_CONFIG.tokensPerMint * mintCount;
        toast.success(i18n.language === 'en' ? 
          `Minted successfully! You will receive ${totalTokens.toLocaleString()} SATS tokens after distribution.` : 
          `铸造成功！您将在分发时收到 ${totalTokens.toLocaleString()} 个SATS代币。`, {
          duration: 5000
        });
  ...
  const projects = [
    {
      ...
      description: i18n.language === 'en' ? 
        'LRC20 token issued on Bitcoin L2 Spark. Minting has ended.' : 
        '比特币L2 Spark上发行的LRC20代币。铸造已结束。',
      status: 'ended',
      ...
      tokenInfo: {
        ...
        tokensPerMint: TOKEN_CONFIG.tokensPerMint
      }
    }
  ];
          ...
          <div className="md:hidden space-y-2">
            {filteredProjects.map((project) => (
              <div key={project.id} className="bg-gray-800/50 rounded-lg border border-gray-700 hover:border-orange-500/50 transition-all duration-300 p-3">
                ...
                <button 
                  onClick={() => {
                    if (project.isTokenLauncher && project.status !== 'ended') {
                      setShowTokenLaunchModal(true);
                    }
                  }}
                  disabled={project.status === 'ended'}
                  className={`w-full py-2 px-3 rounded-lg transition-all font-medium text-xs flex items-center justify-center space-x-1 ${
                ...
                >
                  {project.isTokenLauncher ? (
                    <>
                      ...
                      <span>
                        {project.status === 'ended' 
                          ? (i18n.language === 'en' ? 'Mint Ended' : '铸造已结束')
                          : (i18n.language === 'en' ? 'Mint SATS' : '铸造SATS')
                        }
                      </span>
                    </>
                  ) : (
                ...
                <div className="p-6">
                  ...
                  <div className="flex space-x-3">
                    <button 
                      ...
                      disabled={project.status === 'ended'}
                      className={`flex-1 py-2 px-4 rounded-lg transition-all font-medium text-sm flex items-center justify-center space-x-2 ${
                        project.status === 'ended' 
                          ? 'bg-gray-600 text-gray-300 cursor-not-allowed' 
                          : 'bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:from-orange-600 hover:to-orange-700'
                      }`}
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-6 w-full max-w-md">
            ...
            <div className="bg-gradient-to-r from-orange-500/10 to-yellow-500/10 border border-orange-500/20 rounded-xl p-3 mb-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  <span className="text-gray-400">{i18n.language === 'en' ? 'Per Mint' : '每次获得'}</span>
                  <p className="text-orange-300 font-medium">
                    {TOKEN_CONFIG.tokensPerMint.toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Mint结束提示 */}
            {projects.find(p => p.isTokenLauncher)?.status === 'ended' && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 mb-4">
                <p className="text-red-400 text-sm font-medium text-center">
                  {i18n.language === 'en' ? 'Minting has ended' : '铸造活动已结束'}
                </p>
                <p className="text-red-300 text-xs text-center mt-1">
                  {i18n.language === 'en' ? 'Token distribution will begin soon' : '代币分发即将开始'}
                </p>
              </div>
            )}
            ...
            <div className="flex space-x-3">
              ...
              >
                {projects.find(p => p.isTokenLauncher)?.status === 'ended' ? (
                  <span>{i18n.language === 'en' ? 'Mint Ended' : '铸造已结束'}</span>
                ) : isLaunching ? (
                  <>
